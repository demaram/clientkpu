...
@section('title', 'Form Rekap Piket')

@section('content_header')
    <h1>Form Rekap Piket</h1>
@stop
...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('admin.rekap-piket.form') }}" class="form-inline">
                ...
                <a href="{{ route('admin.rekap-piket.index') }}" class="btn btn-default ml-2">
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Data Piket — {{ $periodStart->translatedFormat('F Y') }}</h3>
        </div>
        <div class="card-body">
            @if($lemburs->isEmpty())
            ...
            @else
                ...
                <div class="mt-3 text-center">
                    <form method="POST" action="{{ route('admin.rekap-piket.approve') }}" class="d-inline">
                        @csrf
                        <input type="hidden" name="month" value="{{ $month }}">
                        <button type="submit" name="action" value="approve" class="btn btn-success"
                            onclick="return confirm('Approve rekap piket bulan {{ $periodStart->translatedFormat('F Y') }}? Total: Rp {{ number_format($totalPay, 0, '.', '.') }}')">
...
@section('js')
    ...
    <script>
        ...
        // Reject a single waiting_approval piket row — mirrors rejectLembur() on
        // the Data Piket page (same reason prompt, same step-approver rule).
        function rejectLemburRow(id) {
            Swal.fire({
                ...
                $.ajax({
                    url: '{{ route("admin.rekap-piket.reject-record") }}',
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        lembur_id: id,
